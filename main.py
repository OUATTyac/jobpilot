...
# --- MODÈLES PYDANTIC (Définis AVANT les endpoints) ---
...
async def generate_promo_image(req: PromoRequest):
    ...
    prompt = f"""Crée une accroche marketing très courte (5-10 mots max) pour cette promotion, soit créatif, libre et pertinent : Artisan: {req.nom}, Promotion: {req.promo}, Fin: {req.date}. Rédige uniquement l'accroche."""
    try:
        response = text_model.generate_content(prompt)
    ...
    overlay = Image.new('RGBA', img.size, (0, 0, 0, 128)); img = Image.alpha_composite(img, overlay)
    draw = ImageDraw.Draw(img)
    try:
        title_font = ImageFont.truetype("font/Poppins-Bold.ttf", 110); subtitle_font = ImageFont.truetype("font/Poppins-Regular.ttf", 60); promo_font = ImageFont.truetype("font/Poppins-Bold.ttf", 70)
    except IOError:
